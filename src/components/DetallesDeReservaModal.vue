<template>
    <mdb-modal :show="show" @close="$emit('close')" size="xl" style="z-index: 1999 !important">
        <mdb-modal-header>
            <mdb-modal-title>Detalles de la Reserva</mdb-modal-title>
        </mdb-modal-header>
                                ...
                                <mdb-card-body style="min-width:920px">
                                    ...
                                    <mdb-row v-for="(item, index) in reserva.descripcionDeTickets" :item="item" :key="index" class="py-2 text-start spainhn-small-text-cells align-items-center border-bottom">
                                        ...
                                        <mdb-col col="2" class="spainhn-small-text-cells ">
                                            <div :class="`text-center rounded py-1 text-white ${coloresEstado[item.estado]}`">{{item.estado}}</div>
                                        </mdb-col>
        ...
        <mdb-modal-footer>
            <mdb-btn color="secondary" @click="$emit('close')">Cerrar</mdb-btn>
        </mdb-modal-footer>
    </mdb-modal>
</template>
...
export default {
    ...
    methods: {
        async doOcultarPrecioEdit() {
            ...
            try {
                await ApiSellerService.editHidePrice(this.localizador, payload)
                this.edicionOk = true;
        ...
        async doReferenceEdit() {
            ...
            try {
                await ApiSellerService.editReference(this.localizador, payload)
                this.edicionOk = true;
        ...
        async loadData() {
            this.reserva = await this.buscarReserva();
            ...
            this.mayorista = await this.obtenerInfoDelMayorista();
            this.listaDePagos = await this.consultarPagos();
        }
    },
    watch: {
        show(newVal) {
